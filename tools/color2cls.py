...
def main():
    ...
    parser.add_argument('--ext', default='npy', choices=['png', 'npy'])
    ...
    palette_tensor = torch.tensor(palette, device=device)
    palette_lab = cv2.cvtColor(palette[None], cv2.COLOR_RGB2Lab)
    palette_lab = torch.tensor(palette_lab, device=device, dtype=torch.float)
    palette_lab = palette_lab.expand(H*W, -1, -1)
    C = palette_lab.size(1)
    i = 0
    for img_path in tqdm(imgs):
        save_path = os.path.join(args.output, os.path.basename(img_path).split('.')[0])
        ...
        x = x.reshape(H*W, 3).unsqueeze(1).expand(-1, C, -1)
        diff = (x-palette_lab).pow(2).sum(dim=2)
        _, idx = diff.min(dim=1)
        idx = idx.reshape(H, W)

        if args.check:
            color_map = palette_tensor[idx]
            Image.fromarray(color_map.cpu().numpy().astype('uint8')).save(save_path+'.png')
        else:
            if args.ext == 'png':
                Image.fromarray(idx.cpu().numpy().astype('uint8')).save(save_path+'.png')
            elif args.ext == 'npy':
                np.save(save_path, idx.cpu().numpy().astype('uint8'))
        i += 1
